...
public class JtpParser {
    public static Node.Nodes parse(String relativeFilePath, byte[] input, URL resourceBase, boolean tag) throws JasperException, IOException {
        ...
        if (tag) {
            ...
            compilationContext = new JspCompilationContext("", tagInfo, options, context, null, runtimeContext, null);
        } else {
            compilationContext = new JspCompilationContext("", options, context, null, runtimeContext);
        }

        ErrorDispatcher errorDispatcher = new ErrorDispatcher(true);

        Compiler compiler = compilationContext.createCompiler();
        compiler.errDispatcher = errorDispatcher;
        compiler.pageInfo = new PageInfo(null, compilationContext);
